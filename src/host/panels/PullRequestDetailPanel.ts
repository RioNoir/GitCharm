...
export class PullRequestDetailPanel {
  ...
  async open(repoId: string, pr: PullRequestSummary): Promise<void> {
    ...
    panel.webview.postMessage({
      type: 'PRDETAIL_INIT', repoId, repoName: meta.name, number: pr.number, summary: pr,
  ...
  private async handleMessage(msg: PrDetailToHostMsg, repoId: string, pr: PullRequestSummary, panel: vscode.WebviewPanel): Promise<void> {
    ...
    switch (msg.type) {
      ...
      case 'PRDETAIL_REQUEST_COMMENTS': {
        const comments = await this.pullRequestManager.listComments(repoId, pr.number);
        post({ type: 'PRDETAIL_COMMENTS_RESULT', comments });
        break;
      }
      ...
      case 'PRDETAIL_REQUEST_FILES': {
        const files = await this.pullRequestManager.listChangedFiles(repoId, pr.number);
        post({ type: 'PRDETAIL_FILES_RESULT', files });
        break;
      }
      ...
      case 'PRDETAIL_REQUEST_COMMITS': {
        const commits = await this.pullRequestManager.listCommits(repoId, pr.number);
        post({ type: 'PRDETAIL_COMMITS_RESULT', commits });
        break;
      }

      case 'PRDETAIL_REQUEST_COMMIT_FILES': {
        const files = await this.pullRequestManager.listCommitFiles(repoId, msg.sha);
        post({ type: 'PRDETAIL_COMMIT_FILES_RESULT', sha: msg.sha, files });
        break;
      }
      ...
      case 'PRDETAIL_CHECKOUT': {
        await this.checkoutPullRequest(repoId, pr, post);
        break;
      }
    }
  }

  private async checkoutPullRequest(repoId: string, pr: PullRequestSummary, post: (m: HostToPrDetailMsg) => void): Promise<void> {
    try {
      const result = await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: `Checking out PR #${pr.number}…`, cancellable: false },
        () => this.pullRequestManager.checkoutPullRequest(repoId, pr),
      );
      ...
      logError('pullrequest-checkout', message, getRawErrorDetail(e));
  ...
  private async openCommitAllChanges(repoId: string, pr: PullRequestSummary, commitSha: string, parentSha: string | undefined): Promise<void> {
    const files = await this.pullRequestManager.listCommitFiles(repoId, commitSha);
    if (files.length === 0) {
      vscode.window.showInformationMessage('No changed files to show.');
      return;
    }
