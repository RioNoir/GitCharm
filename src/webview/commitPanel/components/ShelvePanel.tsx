...
function ShelveRow({ entry, repoId, viewMode, onUnshelve, onUnshelveFile, onDrop, onRename, onOpenFileDiff, isLast }: {
  ...
  return (
    <div style={{ ...rowStyle.root, position: 'relative', ...(isLast ? { borderBottom: 'none' } : {}) }}>
      {/* Guide line from the header's chevron down through the expanded file list, replacing the box border. */}
      {expanded && <div style={rowStyle.expandedGuide} />}
      {/* Header */}
      <div
      ...
      {expanded && (
        <div style={rowStyle.fileList}>
          <GenericFileTree<ShelfFile>
            ...
            isDirOpen={dirPath => !isShelveCollapsed(`${entry.id}:${dirPath}`)}
